...
export const requestWithdrawal = async (req, res) => {
    try {
        ...
        // 2. Fetch rider details to snapshot bank info
        const rider = await Delivery.findById(deliveryBoyId).select("name phone accountHolder accountNumber ifsc").lean();
        ...
        const withdrawal = await Transaction.create({
            ...
            metadata: {
                ...
                destinationDetails: rider?.accountNumber ? `A/C: ${rider.accountNumber} (${rider.ifsc || 'IFSC N/A'})` : "Bank Transfer",
                bankDetails: {
                    accountHolder: rider?.accountHolder || rider?.name,
        ...
        try {
            emitWithdrawalRequestedToAdmins({
                withdrawal,
                user: rider || { name: "Delivery Partner" },
                userModel: "Delivery",
                amount,
            });
        } catch (notifErr) {
            console.error("[deliveryController] Failed to notify admin of delivery withdrawal:", notifErr.message);
        }

        try {
            await invalidateDeliveryEarningsCache(deliveryBoyId);
        } catch (_) {}
...
export const getDeliveryWithdrawals = async (req, res) => {
    try {
        ...
        return handleResponse(res, 200, "Delivery withdrawals fetched successfully", withdrawals);
    } catch (error) {
        return handleResponse(res, 500, error.message);
    }
};
