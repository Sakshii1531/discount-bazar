...
import {
  ...
  bulkSettleAllCashData,
  sendRiderCashReminderData,
...
export const bulkSettleCash = async (req, res) => {
  try {
    const { method, riderIds } = req.body || {};
    const result = await bulkSettleAllCashData({ method, riderIds });
    return handleResponse(res, 200, "Bulk cash settlement completed", result);
  } catch (error) {
    console.error("Bulk settle cash error:", error);
    return handleResponse(res, 500, error.message);
  }
};

export const sendRiderCashReminder = async (req, res) => {
  try {
    const { id: riderId } = req.params;
    const result = await sendRiderCashReminderData(riderId);
    return handleResponse(res, 200, "Cash reminder sent successfully", result);
  } catch (error) {
    console.error("Send cash reminder error:", error);
    return handleResponse(res, 500, error.message);
  }
};
