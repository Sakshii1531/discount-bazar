  ...
  try {
    ...
    try {
      ...
      if (transaction.userModel === "Delivery") {
        import("../../services/delivery/deliveryEarningsService.js").then((m) => {
          m.invalidateDeliveryEarningsCache(transaction.user);
        }).catch(() => {});
      }
    } catch (e) {
      console.error("[walletController] Failed to emit pending review update:", e.message);
    }
