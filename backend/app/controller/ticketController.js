...
export const replyToTicket = async (req, res) => {
    try {
        ...
        try {
            const fromRole = isUserAdmin ? "admin" : "customer";
            const messageIdStr = savedMessage?._id ? String(savedMessage._id) : `msg_${Date.now()}`;
            const payload = {
                fromRole,
                ticketId: String(ticket._id),
                messageId: messageIdStr,
                messageCreatedAt: savedMessage?.createdAt || new Date(),
                userId: ticket.userId,
                customerId: ticket.userId,
                userName: req.user?.name || (isUserAdmin ? "Support Team" : "User"),
                messageText: safeText || (safeMediaUrl ? "Sent an image" : "New message"),
                data: {
                    subject: ticket.subject,
                    ticketId: String(ticket._id),
                    link: `/chat?ticketId=${encodeURIComponent(String(ticket._id))}`,
                },
            };
            ...
            console.error("Failed to emit support ticket notification:", notifErr);
        }
...
export const updateTicketStatus = async (req, res) => {
    try {
        ...
        try {
            emitNotificationEvent(NOTIFICATION_EVENTS.SUPPORT_TICKET_STATUS_UPDATED, {
                ticketId: String(ticket._id),
                userId: ticket.userId,
                customerId: ticket.userId,
                status: ticket.status,
            ...
            console.error("Failed to emit ticket status update notification:", notifErr);
        }
