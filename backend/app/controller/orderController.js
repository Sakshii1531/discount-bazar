...
import { reorderOrder } from "../services/reorderService.js";
...
export const cancelOrder = async (req, res) => {
  try {
    ...
    try {
      ...
      logger.warn("cancelOrder compensation failed", {
        scope: "cancelOrder",
        orderId: order.orderId,
        customerId,
        correlationId: req.correlationId,
...
export const updateOrderStatus = async (req, res) => {
  try {
    ...
    // Strictly enforce seller allowed transitions
    if (role === "seller" && status) {
      const targetLow = String(status).toLowerCase();
      const currentLow = String(order.status || "pending").toLowerCase();

      // Sellers cannot mark orders as out_for_delivery or delivered
      if (targetLow === "out_for_delivery" || targetLow === "delivered") {
        return handleResponse(
          res,
          403,
          "Sellers cannot mark orders as Out for Delivery or Delivered. These steps are handled exclusively by the delivery partner upon OTP verification.",
        );
      }

      if (["packed", "out_for_delivery", "delivered", "cancelled"].includes(currentLow)) {
        return handleResponse(
          res,
          400,
    ...
    if (status === "confirmed" && role === "seller") {
      ...
      try {
        const sid = order.seller?._id || order.seller;
        await emitDeliveryBroadcastForSeller(sid, {
          orderId: canonicalOrderId,
          workflowStatus: order.workflowStatus || "DELIVERY_SEARCH",
          sellerId: sid ? String(sid) : undefined,
          deliverySearchExpiresAt: order.deliverySearchExpiresAt || new Date(Date.now() + 60000).toISOString(),
          preview: {
            pickup: "Seller Store",
            drop: typeof order.address?.address === "string" ? order.address.address : "Customer Address",
            total: order.pricing?.total ?? 0,
            earnings: order.paymentBreakdown?.riderPayoutTotal ?? order.riderEarnings ?? 0,
          },
        });
      } catch (e) {
        logger.warn("updateOrderStatus emitDeliveryBroadcastForSeller failed", {
          orderId: canonicalOrderId,
          error: e.message,
        });
      }
...
const completeReturnAndRefundLegacy = async (order) => {
  ...
  // Note: Seller is ONLY debited for returned product refund. Admin / Platform bears the return delivery commission.
...
/* ===============================
   REORDER ORDER
================================ */
export const reorderOrderController = async (req, res) => {
  try {
    const customerId = req.user?.id;
    const { orderId } = req.params;
    ...
    const isPartialOrEmpty =
      result.summary.addedItemCount === 0 ||
      result.summary.unavailableItems?.length > 0 ||
      result.summary.adjustedItems?.length > 0;

    return handleResponse(
      res,
      200,
      result.message || (result.summary.addedItemCount > 0 ? "Items added to cart" : "No items could be reordered"),
      result,
    );
  } catch (error) {
    if (error.statusCode === 409 && error.details) {
      return handleResponse(res, 409, error.message, error.details);
    }
    return handleResponse(res, error.statusCode || 500, error.message);
  }
};
