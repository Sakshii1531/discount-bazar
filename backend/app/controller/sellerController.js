...
import { emitWithdrawalRequestedToAdmins } from "../services/orderSocketEmitter.js";
...
export const requestWithdrawal = async (req, res) => {
  try {
    ...
    // 2. Fetch seller details to snapshot bank info
    const seller = await Seller.findById(sellerId).select("name shopName phone bankDetails").lean();
    const sBank = seller?.bankDetails || {};
    const computedDest = destinationDetails || (payoutMethod === "UPI" ? "UPI Transfer" : (sBank.accountNumber ? `${sBank.bankName || 'Bank'} A/C: ${sBank.accountNumber}` : "Bank Transfer"));

    // 3. Create Withdrawal Transaction with Payout Destination Details
    const withdrawal = await Transaction.create({
      ...
      metadata: {
        ...
        destinationDetails: computedDest,
        bankDetails: sBank.accountNumber ? {
          accountHolder: sBank.accountHolder || sBank.accountHolderName || seller?.name || seller?.shopName,
          accountNumber: sBank.accountNumber,
          ifsc: sBank.ifscCode || sBank.ifsc,
          bankName: sBank.bankName,
          accountType: sBank.accountType,
          branch: sBank.branch,
          upiId: sBank.upiId
        } : null
      }
    ...
    try {
      emitWithdrawalRequestedToAdmins({
        withdrawal,
        user: seller || { name: "Seller", shopName: "Seller" },
        userModel: "Seller",
        amount,
      });
    } catch (notifErr) {
      console.error("[sellerController] Failed to notify admin of seller withdrawal:", notifErr.message);
    }
