...
function getPayableAmountPaise(target) {
  ...
  const integerRupees = Math.ceil(amountRupees);
  return integerRupees * 100;
}
...
export async function createPaymentOrderForOrderRef({
  ...
  if (existingOpenPayment) {
    if (existingOpenPayment.rawGatewayResponse?.redirectUrl) {
      return {
        ...
        gatewayName: existingOpenPayment.gatewayName,
        duplicate: true,
      };
    }
    if (existingOpenPayment.rawGatewayResponse?.razorpayOrderId) {
      return {
        payment: existingOpenPayment,
        redirectUrl: null,
        merchantOrderId: existingOpenPayment.gatewayOrderId,
        gatewayName: existingOpenPayment.gatewayName,
        razorpayOrderId: existingOpenPayment.rawGatewayResponse.razorpayOrderId,
        keyId: existingOpenPayment.rawGatewayResponse.keyId || process.env.RAZORPAY_KEY_ID,
  ...
  const paymentData = {
    ...
    rawGatewayResponse: {
      ...
      amount: amountPaise,
      ...(initResult.gatewayOrderId ? { razorpayOrderId: initResult.gatewayOrderId } : {}),
      ...(initResult.keyId ? { keyId: initResult.keyId } : {}),
      ...(initResult.gatewayResponse ? { gatewayResponse: initResult.gatewayResponse } : {}),
    },
    statusHistory: [
      {
  ...
  logger.info("payment_order_created", {
    ...
    razorpayOrderId: initResult.gatewayOrderId || null,
  });
...
export async function verifyRazorpayPaymentSignature({
  ...
  const lookupClauses = [];
  if (merchantOrderId) lookupClauses.push({ gatewayOrderId: merchantOrderId });
  if (razorpayOrderId) {
    lookupClauses.push({ "rawGatewayResponse.razorpayOrderId": razorpayOrderId });
    lookupClauses.push({ gatewayOrderId: razorpayOrderId });
  }

  const payment = await Payment.findOne({ $or: lookupClauses });
  if (!payment) {
    const err = new Error("Payment attempt not found");
    err.statusCode = 404;
    throw err;
  ...
  const provider = getActivePaymentProvider();
  if (typeof provider.validatePaymentSignature !== "function") {
    const err = new Error(`Active provider ${provider.providerName} does not support Razorpay signature verification`);
    err.statusCode = 400;
    throw err;
  }

  const isValid = provider.validatePaymentSignature({
    razorpayOrderId,
    razorpayPaymentId,
    razorpaySignature,
  });
  ...
  await transitionPaymentState(payment, {
    nextStatus,
    source: PAYMENT_EVENT_SOURCE.CLIENT_VERIFY,
    reason: `${provider.providerName} client signature verified`,
...
export async function processRazorpayWebhook({
  ...
  if (typeof provider.validateWebhook !== "function") {
    const err = new Error(`Active provider ${provider.providerName} does not support webhook verification`);
    err.statusCode = 400;
    throw err;
  }
  ...
  const decoded = await provider.decodeWebhookPayload({ rawBody, eventIdHeader });
  const eventId = decoded.eventId;
  const payloadHash = crypto
    .createHash("sha256")
    .update(JSON.stringify(decoded.raw))
  ...
  const lookupClauses = [];
  if (decoded.merchantOrderId) lookupClauses.push({ gatewayOrderId: decoded.merchantOrderId });
  if (decoded.gatewayOrderId) {
    lookupClauses.push({ "rawGatewayResponse.razorpayOrderId": decoded.gatewayOrderId });
    lookupClauses.push({ gatewayOrderId: decoded.gatewayOrderId });
  }

  const payment = lookupClauses.length > 0 ? await Payment.findOne({ $or: lookupClauses }) : null;
  if (!payment) {
    return { accepted: true, ignored: true, reason: "Payment attempt not found" };
  }
  ...
  await transitionPaymentState(payment, {
    nextStatus,
    source: PAYMENT_EVENT_SOURCE.WEBHOOK,
    reason: `${provider.providerName} webhook: ${decoded.eventType || decoded.state}`,
    gatewayPaymentId: decoded.transactionId,
    rawGatewayResponse: decoded.raw,
  });
  ...
  await handleOrderSideEffectsFromPaymentStatus(
    payment,
    nextStatus,
    decoded.eventType || decoded.state,
  );
...
// Client payment callback compatibility
export async function verifyClientPaymentCallback(data) {
  ...
  if (provider.providerName === "RAZORPAY") {
    return verifyRazorpayPaymentSignature({
      merchantOrderId: data.merchantOrderId || data.orderRef,
      razorpayOrderId: data.razorpay_order_id || data.gatewayOrderId,
      razorpayPaymentId: data.razorpay_payment_id || data.gatewayPaymentId,
      razorpaySignature: data.razorpay_signature || data.gatewaySignature,
      userId: data.userId,
