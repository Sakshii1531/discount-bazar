...
export async function placeOrderAtomic({
  ...
  try {
    ...
    const walletAmount = Math.max(0, Math.round(Number(normalizedPayload.walletAmount || 0)));
    ...
    // ── Single-seller checkout guard ──────────────────────────────────────────
    // Verify all cart items belong to the same seller before proceeding.
    // This is a last-resort server-side check protecting against:
    //   • Direct API manipulation bypassing the frontend dialog
    //   • Stale/corrupted cart state from old data
    //   • Race conditions where two sellers somehow ended up in the same cart
    if (orderItemsInput.length > 0) {
      const productIds = orderItemsInput
      ...
      const uniqueSellerIds = new Set(
        products.map((p) => String(p.sellerId || "")).filter(Boolean),
      );
      if (uniqueSellerIds.size > 1) {
        const err = new Error(
          "Cart contains products from multiple sellers. Please clear your cart and place a new order.",
        );
        err.statusCode = 400;
        throw err;
      }
    ...
    const checkoutGroup = new CheckoutGroup({
      ...
      coupon: pricingSnapshot.couponSnapshot?.couponId || null,
    });
    ...
    for (let index = 0; index < pricingSnapshot.sellerBreakdownEntries.length; index += 1) {
      ...
      const isOrderFullyWalletCovered =
        proportionateWallet > 0 &&
        (proportionateWallet >= Number(entry.breakdown.grandTotal || 0) ||
          Number(entry.breakdown.payableAmount || entry.breakdown.grandTotal || 0) === 0);
      const effectivePaymentMode = isOrderFullyWalletCovered ? "WALLET" : paymentMode;
      ...
      const order = new Order({
        ...
        paymentStatus:
          isOrderFullyWalletCovered
            ? ORDER_PAYMENT_STATUS.PAID
            : paymentMode === "ONLINE"
              ? ORDER_PAYMENT_STATUS.CREATED
              : ORDER_PAYMENT_STATUS.PENDING_CASH_COLLECTION,
        payment: {
          method: isOrderFullyWalletCovered
            ? "wallet"
            : paymentMode === "ONLINE"
              ? "online"
              : "cash",
          status: isOrderFullyWalletCovered ? "completed" : "pending",
        },
        pricing: {
