...
function buildAggregateBreakdown(sellerBreakdowns = []) {
  const aggregate = {
    ...
    taxTotal: Math.ceil(sumField(sellerBreakdowns, "taxTotal")),
    grandTotal: Math.ceil(sumField(sellerBreakdowns, "grandTotal")),
    ...
    grossTotal: Math.ceil(sumField(sellerBreakdowns, "grossTotal")),
    walletAmount: sumField(sellerBreakdowns, "walletAmount"),
    payableAmount: Math.ceil(sumField(sellerBreakdowns, "payableAmount")),
...
export async function buildCheckoutPricingSnapshot({
  ...
  for (const sellerId of sellerIds) {
    ...
    const sellerTax = Math.ceil(round2((sellerSubtotalAmount * globalTaxRate) / 100));
  ...
  for (const entry of sellerBreakdownEntries) {
    const breakdown = entry?.breakdown;
    if (!breakdown) continue;
    breakdown.grandTotal = Math.ceil(Number(breakdown.grandTotal || 0));
    breakdown.grossTotal = Math.ceil(Number(breakdown.grossTotal || 0));
    breakdown.payableAmount = Math.ceil(Number(breakdown.grandTotal || 0));
    if (breakdown.taxTotal != null) {
      breakdown.taxTotal = Math.ceil(Number(breakdown.taxTotal || 0));
    }
  }
