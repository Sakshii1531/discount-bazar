...
import { decrementCouponUsage } from "./finance/couponService.js";
...
export async function compensateOrderCancellation(order, orderIdString, opts = {}) {
  ...
  if (existing?.checkoutGroupId) {
    ...
    if (activeCount === 0) {
      const targetCouponId = existing.coupon || existing.couponSnapshot?.couponId;
      const groupLock = await CheckoutGroup.findOneAndUpdate(
        {
          checkoutGroupId: existing.checkoutGroupId,
      ...
      if (!groupLock) {
        ...
        const couponIdToReverse = targetCouponId || groupLock.coupon;
        if (couponIdToReverse) {
          await decrementCouponUsage({ couponId: couponIdToReverse });
          logger.info?.("Coupon usage rolled back for cancelled checkout group", {
            checkoutGroupId: existing.checkoutGroupId,
            couponId: couponIdToReverse,
          });
        }
      }
    }
    ...
    const targetCouponId = existing.coupon || existing.couponSnapshot?.couponId;
    if (targetCouponId && !existing.financeFlags?.couponReversalApplied) {
      const orderLock = await Order.findOneAndUpdate(
        {
          _id: existing._id,
      ...
      if (orderLock) {
        await decrementCouponUsage({ couponId: targetCouponId });
        logger.info?.("Coupon usage rolled back for cancelled order", {
