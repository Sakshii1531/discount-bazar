...
/**
 * Atomic decrement of `Coupon.usedCount` with a floor guard at zero.
 *
 * Called during order cancellation compensation (seller rejection, delivery
 * timeout, etc.) to renew the coupon's total availability quota.
 *
 * @param {Object} params
 * @param {string|mongoose.Types.ObjectId} params.couponId Mongo ObjectId of the coupon.
 * @param {*} [params.session] Mongoose session for transactional execution.
 * @returns {Promise<boolean>} True if decremented, false if coupon was not found or already 0.
 */
export async function decrementCouponUsage({ couponId, session = null } = {}) {
  if (!couponId) return false;
  const updateResult = await Coupon.updateOne(
    {
      _id: couponId,
