...
export async function reverseOrderFinanceOnCancellation(
  ...
  try {
    ...
    const customerId = order.customer?._id || order.customer || null;
    ...
    if (order.paymentMode === "ONLINE" && order.financeFlags?.onlinePaymentCaptured) {
      ...
      if (refundAmount > 0) {
        ...
        // Credit customer wallet for the captured online payment
        if (customerId) {
          await creditWallet({
            ...
            bucket: "available",
            session,
            idempotencyKey: `CANCEL-ONLINE-REFUND-${order._id}`,
          });

          const customerWallet = await getOrCreateWallet(OWNER_TYPE.CUSTOMER, customerId, { session });
          await createLedgerEntry(
            {
              orderId: order._id,
              ...
              amount: refundAmount,
              paymentMode: "ONLINE",
              description: `Refund for online payment: ${reason}`,
              reference: order.orderId,
            },
            { session },
          );
              ...
              [
                {
                  ...
                  amount: refundAmount,
                  status: "Settled",
                  description: `Online payment refunded to wallet: ${reason}`,
                  reference: `REFUND-ONLINE-${order.orderId || order._id}`,
                },
              ],
              { session },
            );
          } catch (txErr) {
            // Legacy mirror write is non-blocking
          }
        }
      }
    ...
    if (walletUsed > 0 && customerId) {
      await creditWallet({
        ...
        bucket: "available",
        session,
        idempotencyKey: `CANCEL-WALLET-REFUND-${order._id}`,
      });
          ...
          [
            {
              ...
              status: "Settled",
              description: `Refund for wallet payment: ${reason}`,
              reference: `REFUND-WALLET-${order.orderId || order._id}`,
            },
          ],
          { session },
        );
    ...
    // Fire-and-forget customer notification for wallet refund
    const onlineRefunded = (order.paymentMode === "ONLINE" && order.financeFlags?.onlinePaymentCaptured)
      ? roundCurrency(order.paymentBreakdown?.grandTotal || 0)
      : 0;
    const totalRefunded = roundCurrency(onlineRefunded + walletUsed);

    if (totalRefunded > 0 && customerId) {
      try {
