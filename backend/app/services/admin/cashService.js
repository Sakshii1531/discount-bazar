...
export async function bulkSettleAllCashData({ method = "Cash submission", riderIds = null } = {}) {
  const balancesResult = await getDeliveryCashBalancesData({ page: 1, limit: 1000, skip: 0 });
  let eligibleRiders = (balancesResult.items || []).filter(
    (r) => (r.currentCash || 0) > 0
  );

  if (Array.isArray(riderIds) && riderIds.length > 0) {
    const idSet = new Set(riderIds.map(String));
    eligibleRiders = eligibleRiders.filter((r) => idSet.has(String(r.id)));
  }
  ...
  if (eligibleRiders.length === 0) {
    return {
      ...
      totalAmount: 0,
      settlements: [],
      message: "No outstanding cash balances found to settle",
    };
  }
  ...
  for (const rider of eligibleRiders) {
    const amount = Number(rider.currentCash) || 0;
    ...
    const settlement = await Transaction.create({
      ...
      amount: -Math.abs(amount),
      status: "Settled",
      reference: `CSH-SET-${Date.now()}-${Math.floor(Math.random() * 10000)}`,
      notes: `Bulk Settle - Method: ${method}`,
    });
    ...
    try {
      await Notification.create({
        ...
        recipientModel: "Delivery",
        title: "Cash Settled",
        message: `Admin has collected ₹${amount} cash from you in bulk settlement. Your balance is updated.`,
        type: "payment",
        data: { transactionId: settlement._id },
      });
    } catch (notifErr) {
      console.warn("Notification error during bulk settle:", notifErr.message);
    }
...
export async function sendRiderCashReminderData(riderId) {
  const rider = await Delivery.findById(riderId);
  if (!rider) {
  ...
  const balancesResult = await getDeliveryCashBalancesData({ page: 1, limit: 1000, skip: 0 });
  const riderBalance = (balancesResult.items || []).find((r) => String(r.id) === String(riderId));
  const currentCash = riderBalance ? riderBalance.currentCash : 0;
  ...
  const notif = await Notification.create({
    ...
    message:
      currentCash > 0
        ? `Dear ${rider.name}, you currently have ₹${currentCash.toLocaleString()} in collected COD cash. Please deposit it at the hub as soon as possible.`
        : `Dear ${rider.name}, please visit the hub for cash reconciliation.`,
    type: "alert",
    data: { riderId, currentCash },
  });
