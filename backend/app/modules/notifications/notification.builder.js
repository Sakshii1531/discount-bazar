  ...
  switch (eventType) {
    ...
    case NOTIFICATION_EVENTS.NEW_ORDER:
      return {
        ...
        body: (payload) => {
          const itemAmount =
            payload.itemAmount != null
              ? Number(payload.itemAmount)
              : payload.pricing?.subtotal != null
              ? Number(payload.pricing.subtotal)
              : payload.paymentBreakdown?.productSubtotal != null
              ? Number(payload.paymentBreakdown.productSubtotal)
              : null;

          const netEarnings =
            payload.netEarnings != null
              ? Number(payload.netEarnings)
              : payload.paymentBreakdown?.sellerPayoutTotal != null
              ? Number(payload.paymentBreakdown.sellerPayoutTotal)
              : itemAmount != null
              ? itemAmount
              : null;
    ...
    case NOTIFICATION_EVENTS.SUPPORT_TICKET_STATUS_UPDATED:
      return {
        role: NOTIFICATION_ROLES.CUSTOMER,
        recipientIds: (payload) => normalizeIdList(payload.userId || payload.customerId),
        title: (payload) => {
          const status = String(payload.status || "Updated").toLowerCase();
          if (status === "resolved") return "Support Ticket Resolved";
          if (status === "closed") return "Support Ticket Closed";
          if (status === "processing") return "Support Ticket In Progress";
          return `Support Ticket ${payload.status || "Updated"}`;
        },
        body: (payload) => {
          const status = String(payload.status || "updated").toLowerCase();
          const subject = payload.subject ? ` for "${payload.subject}"` : "";
          if (status === "resolved") return `Your support ticket${subject} has been resolved.`;
          if (status === "closed") return `Your support ticket${subject} has been closed.`;
          if (status === "processing") return `Your support ticket${subject} is now being processed.`;
          return `Your support ticket${subject} status is now ${payload.status}.`;
        },
      };
    ...
    case NOTIFICATION_EVENTS.NEW_SELLER_REGISTRATION:
      return {
        role: NOTIFICATION_ROLES.ADMIN,
        recipientIds: (payload) => normalizeIdList(payload.adminIds),
        title: () => "New Seller Registration",
        body: (payload) =>
          `New seller registration received from ${payload.shopName || payload.name || "a store"}. Pending admin review.`,
      };
    case NOTIFICATION_EVENTS.NEW_DELIVERY_REGISTRATION:
      return {
        role: NOTIFICATION_ROLES.ADMIN,
        recipientIds: (payload) => normalizeIdList(payload.adminIds),
        title: () => "New Delivery Driver Registration",
        body: (payload) =>
          `New delivery driver registration received from ${payload.name || "a rider"}. Pending admin review.`,
      };
    ...
    case NOTIFICATION_EVENTS.WITHDRAWAL_REQUESTED:
      return {
        role: NOTIFICATION_ROLES.ADMIN,
        recipientIds: (payload) => normalizeIdList(payload.adminIds || payload.adminId),
        title: () => "New Money Request 💸",
        body: (payload) => {
          const name = payload.userName || payload.shopName || payload.name || "A partner";
          const amount = payload.amount ? `₹${payload.amount}` : "funds";
          return `${name} requested a withdrawal of ${amount}.`;
        },
      };
    case NOTIFICATION_EVENTS.WITHDRAWAL_STATUS_UPDATED:
      return {
        role: NOTIFICATION_ROLES.SELLER,
        recipientIds: (payload) => normalizeIdList(payload.userId || payload.sellerId),
        title: (payload) => {
          const status = String(payload.status || "Updated").toLowerCase();
          if (status === "approved" || status === "completed" || status === "settled") {
            return "Payout Processed 💰";
          }
          if (status === "rejected") {
            return "Payout Request Rejected ❌";
          }
          return `Payout Status: ${payload.status || "Updated"}`;
        },
        body: (payload) => {
          const amount = payload.amount ? `₹${payload.amount}` : "";
          const status = String(payload.status || "").toLowerCase();
          if (status === "approved" || status === "completed" || status === "settled") {
            return `Your withdrawal request of ${amount} has been processed and transferred to your bank account.`;
          }
          if (status === "rejected") {
            return `Your withdrawal request of ${amount} was rejected.${payload.reason ? ` Reason: ${payload.reason}` : ""}`;
          }
          return `Your withdrawal request of ${amount} status is now ${payload.status}.`;
        },
      };
    default:
      return null;
...
function eventData(eventType, payload = {}, role) {
  ...
  if (eventType === NOTIFICATION_EVENTS.NEW_SELLER_REGISTRATION) {
    const baseUrl = getFrontendBaseUrl();
    const sellerId = String(payload.sellerId || "").trim() || undefined;
    return {
      eventType,
      sellerId,
      link: `${baseUrl}/admin/sellers/pending`,
      ...(payload.data || {}),
    };
  }

  if (eventType === NOTIFICATION_EVENTS.NEW_DELIVERY_REGISTRATION) {
    const baseUrl = getFrontendBaseUrl();
    const deliveryId = String(payload.deliveryId || "").trim() || undefined;
    return {
      eventType,
      deliveryId,
      link: `${baseUrl}/admin/delivery-boys/pending`,
      ...(payload.data || {}),
    };
  }

  if (eventType === NOTIFICATION_EVENTS.WITHDRAWAL_REQUESTED) {
    const baseUrl = getFrontendBaseUrl();
    const tab = payload.tab || (payload.userModel === "Seller" ? "sellers" : "delivery");
    return {
      eventType,
      tab,
      ...
      link: `${baseUrl}/admin/withdrawals?tab=${tab}`,
      ...(payload.data || {}),
    };
  }
  ...
  if (
    ...
    eventType === NOTIFICATION_EVENTS.SUPPORT_TICKET_STATUS_UPDATED
  ) {
  ...
  return {
    ...
    checkoutGroupId,
    link,
    ...(itemAmount != null && !isNaN(itemAmount) ? { itemAmount: Math.round(itemAmount) } : {}),
    ...(netEarnings != null && !isNaN(netEarnings) ? { netEarnings: Math.round(netEarnings) } : {}),
    ...(payload.data || {}),
  };
}
