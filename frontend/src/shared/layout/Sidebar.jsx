      ...
      <div className="space-y-1">
        <button
          ...
          <div className="flex items-center space-x-2 z-10">
            {badgeCount > 0 && (
              <span className="min-w-5 h-5 px-1.5 rounded-full bg-rose-500 text-white text-[10px] font-black flex items-center justify-center shadow-lg shadow-rose-500/30 ring-2 ring-[#0a0c10]">
                {badgeLabel}
              </span>
            )}
            <div
          ...
          <div className="pl-9 pr-3 py-1 space-y-1 animate-in slide-in-from-top-2 fade-in duration-500">
            {item.children.map((child) => {
              ...
              const effectiveChildCount =
                childBadgeCount > 0
                  ? childBadgeCount
                  : (isSupportTicket ? badgeCount : 0);
              const showChildBadge = effectiveChildCount > 0;
              const childBadgeLabel =
                effectiveChildCount > 99 ? "99+" : String(effectiveChildCount);
