...
const ProductDetailSheet = () => {
    ...
    const isOutOfStock = useMemo(() => {
        ...
        if (selectedVariant) {
            if (selectedVariant.status === "out_of_stock" || selectedVariant.status === "OUT_OF_STOCK" || selectedVariant.inStock === false) return true;
        ...
        if (variants.length > 0 && !selectedVariant) {
            const allOut = variants.every((v) => isVariantOutOfStock(v));
    ...
    const availableStock = useMemo(() => {
        ...
        return masterStock;
    }, [selectedProduct, selectedVariant, isOutOfStock, masterStock]);
                                            ...
                                            >
                                                ...
                                                <div className="flex gap-3 flex-wrap">
                                                    {selectedProduct.variants.map((v, idx) => {
                                                        const isVarOut = isVariantOutOfStock(v);
                                                        return (
                                                            <motion.button
                                                                key={idx}
                                                                ...
                                                                className={cn(
                                                                    'px-4 py-2 font-[600] rounded-lg text-[13px] transition-all border-2 flex items-center gap-1.5',
                                                            ...
                                                            >
                                                                <span>{v.name}</span>
                                                                {isVarOut && (
                                                                    <span className="text-[10px] font-bold text-red-500 bg-red-50 px-1 py-0.5 rounded">Out</span>
                                                                )}
                                                            </motion.button>
                                                        );
                                                    })}
                                            ...
                                            {selectedProduct.variants.map((v, idx) => {
                                                ...
                                                return (
                                                    <motion.button
                                                        ...
                                                        onClick={() => setSelectedVariant(v)}
                                                        className={cn(
                                                            "flex-shrink-0 px-5 py-2.5 font-bold rounded-xl text-sm transition-all relative border-2 flex items-center gap-1.5",
                                                            selectedVariant?.sku === v.sku
