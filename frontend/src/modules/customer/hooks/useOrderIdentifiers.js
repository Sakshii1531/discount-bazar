...
export function resolveOrderIdentifiers(order, routeParam) {
  ...
  const mongoId = norm(order?._id || order?.id);

  const canonicalOrderId = canonical || route || mongoId || null;
  const lookupId = canonical || group || route || mongoId || null;

  const identifiers = Array.from(
    new Set([route, canonical, group, mongoId].filter(Boolean)),
  );

  const extraRoomId = canonical && canonical !== route ? canonical : (mongoId && mongoId !== route ? mongoId : "");
...
export function useOrderIdentifiers(routeParam, order) {
  const resolved = useMemo(
    ...
    [routeParam, order?.orderId, order?.checkoutGroupId, order?._id, order?.id],
  );
