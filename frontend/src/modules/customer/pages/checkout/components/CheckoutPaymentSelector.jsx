  ...
  return (
    <>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">
                  Available: {formatCurrencyInteger(walletBalance)}
                </p>
              </div>
            </div>
            <button
            ...
            <motion.div
              ...
              <div className="flex justify-between items-center bg-brand-50/50 p-2 rounded-xl">
                ...
                <span className="text-[13px] font-black text-primary">
                  {formatCurrencyInteger(walletAmountToUse)}
                </span>
              </div>
            </motion.div>
          )}
      ...
      {isFullyWalletCovered ? (
        <motion.div className="bg-white rounded-2xl p-4 shadow-sm border border-brand-100 bg-brand-50/30">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-full bg-brand-100 flex items-center justify-center text-primary shrink-0">
              <Wallet size={20} />
            </div>
            <div className="flex-1">
              <h3 className="font-black text-slate-800 text-sm tracking-tight">
                Paid via Wallet (100% Covered)
              </h3>
              <p className="text-xs text-slate-500 font-medium">
                No cash or online payment required on delivery
              </p>
            </div>
            <span className="px-2.5 py-1 bg-brand-100 text-primary text-xs font-black rounded-full shrink-0">
              ₹0 to pay
            </span>
          </div>
        </motion.div>
        ...
        <motion.div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
          <h3 className="font-black text-slate-800 mb-4 uppercase text-sm tracking-widest">
            {useWallet && walletAmountToUse > 0
              ? "Payment Method for Remaining Amount"
              : "Payment Method"}
          </h3>
