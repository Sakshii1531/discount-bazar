...
const OrderDetailPage = () => {
  ...
  const handleReorder = async () => {
    const orderKey = order?._id || order?.orderId || orderId;
    if (!orderKey || isReordering) return;
  ...
  const fetchOrderDetails = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const ord = response.data?.result || response.data?.data;
      if (!ord) return;
      setOrder(ord);

      try {
        const retRes = await customerApi.getReturnDetails(resolveOrderLookupId(ord));
        const ret = retRes.data?.result || retRes.data?.data;
  ...
  useEffect(() => {
    ...
    const offStatus = onOrderStatusUpdate(getToken, (payload) => {
      // Guard: only process updates that belong to this order. The backend emits
      // order:status:update to both the order room AND the customer personal room,
      // so a customer with multiple orders could receive cross-order events here.
      if (payload?.orderId && !matchesOrderIdentifier(payload.orderId, identifiersRef.current)) return;
      ...
      if (ws || st) {
        setOrder((prev) => {
          ...
          return {
            ...
            ...(ws === "PICKUP_READY" && { status: "confirmed" }),
    ...
    // Secondary real-time pathway: delivery:otp:validated is emitted directly
    // to the customer's personal room (customer:${userId}) on OTP confirmation,
    // so it arrives even if the order room join hadn't fully settled.
    const offOtpValidated = onDeliveryOtpValidated(getToken, (payload) => {
      ...
      setOrder((prev) => {
        ...
        return {
          ...
          workflowStatus: "DELIVERED",
          status: "delivered",
          deliveredAt: payload?.deliveredAt || prev.deliveredAt || new Date().toISOString(),
        };
      });
      refresh();
    });
  ...
  useEffect(() => {
    ...
    const roomsToJoin = Array.from(new Set([
      canonicalOrderId,
    ...
    return () => {
      roomsToJoin.forEach((room) => {
        if (room && room !== orderId) {
          leaveOrderRoom(room, getToken);
        }
      });
    };
  }, [orderId, canonicalOrderId, extraRoomId, order?._id, order?.orderId]);
  ...
  // Auto-update order status: smart polling every 3.5 seconds while order is active
  // Also refreshes immediately whenever tab becomes visible again
  useEffect(() => {
    if (!orderId) return undefined;
    const isTerminal =
      order &&
    ...
    const handleVisibilityChange = () => {
      if (!document.hidden && !refreshRef.current.inFlight) {
        fetchOrderDetails(true);
      }
    };
  ...
  }, [orderId, order?.status, order?.workflowStatus, fetchOrderDetails]);

  const fallbackRiderCoords = order?.deliveryBoy?.location?.coordinates;
  const dbRiderLocation = useMemo(() => {
    if (Array.isArray(fallbackRiderCoords) && fallbackRiderCoords.length === 2) {
      const [lng, lat] = fallbackRiderCoords.map(Number);
  ...
  const effectiveRiderLocation = (liveLocation && typeof liveLocation.lat === "number" && typeof liveLocation.lng === "number")
    ? liveLocation
    : dbRiderLocation;
  ...
  const handleOpenInMaps = () => {
    ...
    const rider = effectiveRiderLocation;
  ...
  const handleRetryPayment = async () => {
    try {
      ...
      // 2. Razorpay modal
      if (response.data.success && response.data.result?.razorpayOrderId) {
        ...
        const razorpayOptions = {
          ...
          handler: async function (razorpayResponse) {
            try {
              ...
              if (verifyRes.data.success) {
                ...
                if (typeof fetchOrderDetails === "function") {
                  fetchOrderDetails();
                } else {
          ...
          modal: {
            ondismiss: function () {
              toast.info(
                "Payment was not completed. You can retry when ready.",
              );
            },
          },
        };
        ...
        rzp.on("payment.failed", function (failResponse) {
          toast.error(
            failResponse.error?.description ||
              "Payment failed. You can retry anytime.",
          );
        });
        rzp.open();
        return;
  ...
  return (
    <div className="bg-slate-50 pb-1 font-sans">
      {/* Minimal Header */}
      ...
      <div className="max-w-2xl mx-auto px-4 pt-3 pb-1 space-y-4">
            ...
            <div className="relative z-10 flex items-center justify-between gap-4">
              <div className="flex-1">
                ...
                <p className="text-xs text-brand-700 font-medium leading-relaxed">
                  Complete your payment of <span className="font-bold">{formatCurrencyInteger(order.pricing?.total)}</span> to proceed with this order.
                </p>
              </div>
              <button
        ...
        >
          ...
          {/* Payment Method */}
          {(() => {
            const rawMethod = (order.payment?.method || order.paymentMode || order.paymentMethod || "").toLowerCase();
            const walletAmt = Number(order.walletAmount || order.pricing?.walletAmount || order.paymentBreakdown?.walletDebitedAmount || 0);
            const totalAmt = Number(order.pricing?.total || order.payableAmount || order.paymentBreakdown?.grandTotal || 0);
            const isFullyWallet = rawMethod === "wallet" || (walletAmt > 0 && (walletAmt >= totalAmt || totalAmt === 0));
            const isPartialWallet = walletAmt > 0 && walletAmt < totalAmt;

            let paymentTitle = "Cash on Delivery";
            if (isFullyWallet) {
              paymentTitle = "Wallet";
            } else if (isPartialWallet) {
              if (rawMethod === "online" || rawMethod === "razorpay" || rawMethod === "upi" || rawMethod === "card") {
                paymentTitle = "Wallet + Online Payment";
              } else {
                paymentTitle = "Wallet + Cash on Delivery";
              }
            } else if (rawMethod === "online" || rawMethod === "razorpay" || rawMethod === "upi" || rawMethod === "card" || rawMethod === "stripe") {
              paymentTitle = "Online Payment";
            } else if (rawMethod === "cash" || rawMethod === "cod") {
              paymentTitle = "Cash on Delivery";
            } else if (order.payment?.method) {
              paymentTitle = order.payment.method;
            }
        ...
        >
          <button
            id="order-detail-reorder-btn"
            type="button"
            onClick={handleReorder}
            disabled={isReordering}
            className="w-full py-3.5 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-black transition-all flex items-center justify-center gap-2 text-sm shadow-md hover:shadow-lg active:scale-[0.98] disabled:opacity-70 cursor-pointer"
          >
            {isReordering ? (
              <>
        ...
        {/* Return Section - Only shown when a return is in progress */}
        {order?.status !== "cancelled" && returnDetails && returnDetails.returnStatus && returnDetails.returnStatus !== "none" && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
