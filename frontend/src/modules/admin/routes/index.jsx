...
import { usePendingReview } from "@core/context/PendingReviewContext";
...
const AdminRoutes = () => {
  ...
  const { pendingSellers, pendingDrivers, pendingProducts, pendingWithdrawals } = usePendingReview();

  const navItemsWithBadges = React.useMemo(() => {
    const supportCount = Number.isFinite(totalUnread) ? totalUnread : 0;
    const sellersCount = Number.isFinite(pendingSellers) ? pendingSellers : 0;
    const driversCount = Number.isFinite(pendingDrivers) ? pendingDrivers : 0;
    const productsCount = Number.isFinite(pendingProducts) ? pendingProducts : 0;
    const withdrawalsCount = Number.isFinite(pendingWithdrawals) ? pendingWithdrawals : 0;

    return navItems.map((item) => {
      if (item?.label === "Products") {
        return {
          ...item,
          badgeCount: productsCount,
        };
      }
      ...
      if (item?.label === "Sellers") {
        return {
          ...item,
          badgeCount: sellersCount,
          children: item.children?.map((child) =>
            child.path === "/admin/sellers/pending"
              ? { ...child, badgeCount: sellersCount }
              : child,
          ),
        };
      }
      ...
      if (item?.label === "Delivery Drivers") {
        return {
          ...item,
          badgeCount: driversCount,
          children: item.children?.map((child) =>
            child.path === "/admin/delivery-boys/pending"
              ? { ...child, badgeCount: driversCount }
              : child,
          ),
        };
      }
  ...
  }, [totalUnread, pendingSellers, pendingDrivers, pendingProducts, pendingWithdrawals]);
  ...
  return (
    <DashboardLayout navItems={navItemsWithBadges} title="Admin Center">
      <Routes>
        ...
        <Route path="/seller%20locations" element={<Navigate to="/admin/seller-locations" replace />} />
        <Route path="/seller locations" element={<Navigate to="/admin/seller-locations" replace />} />
        <Route path="/sellers/locations" element={<Navigate to="/admin/seller-locations" replace />} />
