    ...
    const handleExport = async () => {
        try {
            ...
            try {
                ...
                console.warn("API fetch error, falling back to loaded transactions", apiErr);
            }
            ...
            const formatted = items.map(t => {
                ...
                return {
                    ...
                    "Shop Name": t.user?.shopName || t.user?.name || 'Unknown',
                    "Transaction Type": type,
                    "Amount (INR)": t.amount || 0,
                    "Commission (INR)": t.order?.pricing?.platformFee || 0,
                    "Net Payable (INR)": t.amount || 0,
                    "Status": (t.status || 'Unknown').toUpperCase(),
                    "Reference ID": t.reference || t._id || 'N/A',
                    "Order ID": t.order?.orderId || 'N/A',
                    "Payment Method": t.paymentMethod || 'Wallet'
                };
            });

            exportToCSV(formatted, 'Master_Seller_Ledger');
