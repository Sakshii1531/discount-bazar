...
const CashCollection = () => {
    ...
    const [remindingRiderId, setRemindingRiderId] = useState(null);
    ...
    const ridersWithCash = useMemo(() => {
        return (ridersCashData || []).filter(r => (r.currentCash || 0) > 0);
    }, [ridersCashData]);

    const totalCashToSettle = useMemo(() => {
        return ridersWithCash.reduce((acc, r) => acc + (r.currentCash || 0), 0);
    }, [ridersWithCash]);

    const handleOpenBulkSettle = () => {
        if (!ridersWithCash || ridersWithCash.length === 0) {
            toast.error("No delivery partners currently have outstanding cash in hand to settle");
            return;
        }
    ...
    const confirmBulkSettlement = async () => {
        try {
            ...
            const res = await adminApi.bulkSettleCash({ method: 'Cash submission' });
            if (res.data.success) {
                const count = res.data.result?.count ?? ridersWithCash.length;
                const total = res.data.result?.totalAmount ?? totalCashToSettle;
                toast.success(`Successfully settled ₹${total.toLocaleString()} across ${count} delivery partners.`);
                setIsBulkSettleModalOpen(false);
                fetchData(ridersPage, historyPage);
            } else {
                toast.error(res.data.message || "Failed to process bulk settlement");
            }
        } catch (error) {
            console.error("Bulk settlement error:", error);
            toast.error(error.response?.data?.message || "Failed to process bulk settlement");
        } finally {
            setIsBulkSettling(false);
        }
    };

    const handleSendReminder = async (rider) => {
        try {
            setRemindingRiderId(rider.id);
            const res = await adminApi.sendRiderCashReminder(rider.id);
            if (res?.data?.success) {
                toast.success(`Cash deposit reminder sent to ${rider.name}`);
            } else {
    ...
    const handleExport = async () => {
        try {
            ...
            if (activeTab === 'live_balances') {
                let riders = ridersCashData;
                try {
                    const res = await adminApi.getDeliveryCashBalances({ page: 1, limit: 5000, search: searchTerm.trim() || undefined });
                    if (res?.data?.success) {
                        const payload = res.data.result || {};
                        const items = Array.isArray(payload.items) ? payload.items : (payload.riders || []);
                        if (items.length > 0) riders = items;
                    }
                } catch (e) {
                    console.warn("Fallback to loaded riders for export", e);
                }

                if (!riders || riders.length === 0) {
                    toast.error('No cash balance records to export');
                    return;
                }

                const formatted = riders.map(r => ({
                    "Rider ID": r.id || '',
                    "Rider Name": r.name || 'Unknown',
                    "Delivered Orders": r.totalOrders || 0,
                    "Pending Orders": r.pendingOrders || 0,
                    "Current Cash (INR)": r.currentCash || 0,
                    "Limit (INR)": r.limit || 0,
                    "Status": (r.status || 'safe').toUpperCase(),
                    "Last Settlement": r.lastSettlement !== 'Never' && r.lastSettlement ? new Date(r.lastSettlement).toLocaleDateString('en-GB') : 'No History'
                }));
                exportToCSV(formatted, 'Cash_Collection_Balances');
                toast.success('Cash balances exported successfully');
            } else {
                let history = historyData;
                try {
                    const res = await adminApi.getCashSettlementHistory({ page: 1, limit: 5000, search: searchTerm.trim() || undefined });
                    if (res?.data?.success) {
                        const payload = res.data.result || {};
                        const items = Array.isArray(payload.items) ? payload.items : (res.data.results || res.data.result || []);
                        if (items.length > 0) history = items;
                    }
                } catch (e) {
                    console.warn("Fallback to loaded history for export", e);
                }

                if (!history || history.length === 0) {
                    toast.error('No settlement history to export');
                    return;
                }

                const formatted = history.map(h => ({
                    "Date": h.createdAt ? new Date(h.createdAt).toLocaleDateString('en-GB') : '',
                    "Rider": h.riderName || h.rider?.name || 'Unknown',
                    "Amount (INR)": h.amount || 0,
                    "Settled By": h.settledByName || 'Admin',
                    "Reference ID": h.reference || h._id || '',
                    "Status": (h.status || 'SETTLED').toUpperCase()
                }));
                exportToCSV(formatted, 'Cash_Settlement_History');
    ...
    return (
        <div className="ds-section-spacing animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12 pt-6 relative z-10">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 px-1">
                ...
                <div className="flex items-center gap-3">
                    <button
                        ...
                        className="flex items-center gap-2 px-5 py-3 bg-white ring-1 ring-slate-200 text-slate-700 rounded-2xl text-xs font-bold hover:bg-slate-50 transition-all shadow-sm cursor-pointer active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        {isExporting ? <RotateCw className="h-4 w-4 animate-spin text-brand-500" /> : <Download className="h-4 w-4" />}
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 px-5 py-3 bg-slate-900 text-white rounded-2xl text-xs font-bold hover:bg-slate-800 transition-all shadow-lg active:scale-95 shadow-slate-200 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        {isBulkSettling ? <RotateCw className="h-4 w-4 animate-spin text-white" /> : <CheckCircle2 className="h-4 w-4 text-slate-100" />}
                        {isBulkSettling ? 'SETTLING ALL...' : 'BULK SETTLE ALL'}
                    </button>
                </div>
            </div>
                                        ...
                                        <td className="px-6 py-6 text-right pr-8">
                                            <div className="flex items-center justify-end gap-2">
                                                <button
                                                    ...
                                                    className="px-4 py-2 bg-brand-50 text-brand-600 rounded-xl text-[10px] font-black hover:bg-black hover:text-white transition-all shadow-sm active:scale-95 uppercase tracking-widest cursor-pointer"
                                                >
                                                    Settle
                                                </button>
                                                <button
                                                    type="button"
                                                    onClick={() => handleSendReminder(rider)}
                                                    disabled={remindingRiderId === rider.id}
                                                    title={`Send cash deposit reminder to ${rider.name}`}
                                                    className="p-2.5 bg-slate-50 text-slate-400 rounded-xl hover:bg-slate-900 hover:text-white transition-all active:scale-95 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                                                >
                                                    {remindingRiderId === rider.id ? (
                                                ...
                                                <button
                                                    ...
                                                    title={`View details for ${rider.name}`}
                                                    className="p-2.5 bg-slate-50 text-slate-400 rounded-xl hover:bg-slate-900 hover:text-white transition-all active:scale-95 cursor-pointer"
                                                >
            ...
            {/* Bulk Settle Modal */}
            <Modal
                isOpen={isBulkSettleModalOpen}
                onClose={() => !isBulkSettling && setIsBulkSettleModalOpen(false)}
                title="Bulk Settle All Cash Balances"
                size="md"
            >
                <div className="space-y-4 py-2">
                    <p className="text-xs text-slate-600 leading-relaxed">
                        Are you sure you want to settle all outstanding physical cash balances collected by delivery partners?
                    </p>

                    <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl space-y-3">
                        <div className="flex justify-between items-center text-xs">
                            <span className="text-slate-500 font-medium">Delivery Partners with Cash:</span>
                            <span className="font-bold text-slate-900">{ridersWithCash.length} riders</span>
                        </div>
                        <div className="flex justify-between items-center text-xs">
                            <span className="text-slate-500 font-medium">Total Cash to Settle:</span>
                            <span className="text-base font-extrabold text-emerald-600">₹{totalCashToSettle.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between items-center text-xs border-t border-slate-200/60 pt-2">
                            ...
                            <span className="font-bold text-slate-700">Cash Submission</span>
                        </div>
                    </div>

                    <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-2.5 text-xs text-amber-800">
                        <AlertTriangle className="h-4 w-4 shrink-0 text-amber-600 mt-0.5" />
                        <span>This will record settlement transactions for all riders and clear their cash-in-hand balances.</span>
                    </div>
                    ...
                    <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                        ...
                        <button
                            type="button"
                            disabled={isBulkSettling}
                            onClick={confirmBulkSettlement}
                            className="flex items-center gap-2 px-5 py-2.5 bg-slate-900 text-white font-bold text-xs rounded-xl shadow-lg hover:bg-slate-800 active:scale-95 transition-all cursor-pointer disabled:opacity-50"
                        >
                            {isBulkSettling && <RotateCw className="h-3.5 w-3.5 animate-spin text-white" />}
                            {isBulkSettling ? 'Settling All...' : 'Confirm Bulk Settle'}
                        </button>
                    </div>
                </div>
            </Modal>
