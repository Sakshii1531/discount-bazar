...
const getListingImageForReturn = (ret) => {
    ...
    const returnItems = toSafeArray(ret.returnItems);
    const returnedItem = returnItems[0];
    if (returnedItem?.image) return returnedItem.image;

    const items = toSafeArray(ret.items);
    if (items.length > 0) {
        if (typeof returnedItem?.itemIndex === "number" && items[returnedItem.itemIndex]?.image) {
            return items[returnedItem.itemIndex].image;
        }
        ...
        if (items[0]?.image) return items[0].image;
    }
...
const Returns = () => {
    ...
    useEffect(() => {
        ...
        // Listen for newly requested returns to refresh list in real time
        const unsubscribeReturnReq = onSellerReturnRequested(getToken, () => {
            fetchReturns();
        });

        return () => {
            if (typeof unsubscribeDrop === "function") unsubscribeDrop();
            if (typeof unsubscribeReturnReq === "function") unsubscribeReturnReq();
        };
                        ...
                        >
                            ...
                            <div className="px-4 py-4 sm:px-6 sm:py-5 overflow-y-auto overscroll-contain flex-1 space-y-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    {toSafeArray(selectedReturn.returnImages).length > 0 && (
                                        <div className="pt-2 space-y-2">
                                            <p className="text-xs font-bold text-slate-600 uppercase tracking-widest">
                                                Customer Photos ({toSafeArray(selectedReturn.returnImages).length})
                                            </p>
                                            <div className="flex gap-2 overflow-x-auto pb-2">
                                                {toSafeArray(selectedReturn.returnImages).map((img, idx) => (
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="space-y-2">
                                        {(() => {
                                            const returnItems = toSafeArray(selectedReturn.returnItems);
