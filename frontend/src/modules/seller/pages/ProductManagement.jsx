    ...
    try {
      ...
      if (editingItem) {
        variantsToSubmit = (formData.variants || []).map((v) => {
          ...
          return {
            ...v,
            stock: original ? Number(original.stock ?? 0) : Number(v.stock ?? 0),
          };
        });
        computedStock = variantsToSubmit.reduce((sum, v) => sum + Math.max(0, Number(v.stock) || 0), 0);
      }

      data.append("stock", computedStock);
    ...
    <div className="space-y-6 pb-16">
      ...
      <Card className="border-none shadow-sm ring-1 ring-slate-100 p-3 bg-white/60 backdrop-blur-xl">
        <div className="flex flex-col lg:flex-row gap-3 items-center">
          ...
          <div className="grid grid-cols-2 sm:flex sm:flex-wrap lg:flex-nowrap gap-2 w-full lg:w-auto min-w-0">
            <select
              value={filterCategory}
              onChange={(e) => setFilterCategory(e.target.value)}
              className="w-full sm:w-auto min-w-0 px-2.5 sm:px-4 py-2.5 bg-white ring-1 ring-slate-200 rounded-xl text-xs font-bold text-slate-700 focus:ring-2 focus:ring-primary/5 outline-none appearance-none cursor-pointer truncate"
            >
            ...
            <div className="relative min-w-0">
              <button
                ...
                className="flex items-center justify-center space-x-1.5 sm:space-x-2 w-full px-2.5 sm:px-4 py-2.5 bg-white ring-1 ring-slate-200 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-50 transition-all"
              >
              ...
              {isFilterOpen && (
                <div
                  ...
                  className="absolute z-[9999] left-0 sm:left-auto sm:right-0 top-full mt-2 w-64 max-w-[calc(100vw-2rem)] rounded-xl border border-slate-200 bg-white shadow-xl p-4 space-y-3"
                >
                  <div>
                        ...
                        <div>
                          ...
                          {editingItem && (
                            <p className="text-[11px] text-amber-600 font-medium mt-0.5">
                              Stock is locked in Edit. Use <span className="font-bold underline">Stock Management</span> to adjust inventory.
                            </p>
                          )}
                        </div>
                        <button
