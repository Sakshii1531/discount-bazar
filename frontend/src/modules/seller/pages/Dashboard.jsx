...
const Dashboard = () => {
  ...
  const salesTrendStr = statsData?.overview?.salesTrend || "0%";
  const salesTrendNum = parseFloat(salesTrendStr) || 0;
  const salesChangeType =
    salesTrendNum > 0 ? "increase" : salesTrendNum < 0 ? "decrease" : "neutral";

  const ordersTrendStr = statsData?.overview?.ordersTrend || "0%";
  const ordersTrendNum = parseFloat(ordersTrendStr) || 0;
  const ordersChangeType =
    ordersTrendNum > 0 ? "increase" : ordersTrendNum < 0 ? "decrease" : "neutral";

  const stats = [
    {
      label: "Total Revenue",
      value: statsData?.overview?.totalSales || "₹0",
      change: salesTrendNum !== 0 ? `${salesTrendNum > 0 ? "+" : ""}${salesTrendNum}%` : "0%",
    ...
    },
    {
      ...
      change: ordersTrendNum !== 0 ? `${ordersTrendNum > 0 ? "+" : ""}${ordersTrendNum}%` : "0%",
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  >
                    {stat.changeType !== "neutral" ? (
                      <TrendingUp
                        className={cn("h-3 w-3", stat.changeType === "decrease" && "rotate-180")}
                      />
                    ) : null}
                    {stat.change}
                  </span>
