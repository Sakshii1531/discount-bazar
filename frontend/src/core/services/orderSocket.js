...
export function onAdminPendingUpdate(getToken, handler) {
  const s = getOrderSocket(getToken);
  if (!s || typeof handler !== "function") return () => {};
  s.on("admin:pending:update", handler);
  return () => s.off("admin:pending:update", handler);
}

export function onAdminWithdrawalNew(getToken, handler) {
  const s = getOrderSocket(getToken);
  if (!s || typeof handler !== "function") return () => {};
  s.on("admin:withdrawal:new", handler);
  return () => s.off("admin:withdrawal:new", handler);
}

export function onSellerApproved(getToken, handler, sellerId) {
  const s = getOrderSocket(getToken);
  if (!s || typeof handler !== "function") return () => {};

  const handleEvent = (payload) => {
    if (sellerId && payload?.sellerId && String(payload.sellerId) !== String(sellerId)) {
      return;
    }
    handler(payload);
  };

  s.on("seller:approved", handleEvent);
  const specificEvent = sellerId ? `seller:approved:${sellerId}` : null;
  if (specificEvent) {
    s.on(specificEvent, handleEvent);
  }

  return () => {
    s.off("seller:approved", handleEvent);
    if (specificEvent) {
      s.off(specificEvent, handleEvent);
    }
  };
}

export function onDeliveryApproved(getToken, handler, deliveryId, deliveryPhone) {
  const s = getOrderSocket(getToken);
  if (!s || typeof handler !== "function") return () => {};

  const cleanPhone = deliveryPhone ? String(deliveryPhone).replace(/\D/g, "") : "";

  const handleEvent = (payload) => {
    const payloadId = payload?.deliveryId ? String(payload.deliveryId) : "";
    const payloadPhone = payload?.phone ? String(payload.phone).replace(/\D/g, "") : "";
    const matchId = deliveryId && payloadId && String(deliveryId) === payloadId;
    const matchPhone = cleanPhone && payloadPhone && cleanPhone === payloadPhone;

    if (deliveryId || cleanPhone) {
      if (!matchId && !matchPhone) return;
    }
    handler(payload);
  };

  s.on("delivery:approved", handleEvent);
  const specificIdEvent = deliveryId ? `delivery:approved:${deliveryId}` : null;
  if (specificIdEvent) {
    s.on(specificIdEvent, handleEvent);
  }
  const specificPhoneEvent = cleanPhone ? `delivery:approved:${cleanPhone}` : null;
  if (specificPhoneEvent) {
    s.on(specificPhoneEvent, handleEvent);
  }

  return () => {
    s.off("delivery:approved", handleEvent);
    if (specificIdEvent) {
      s.off(specificIdEvent, handleEvent);
    }
    if (specificPhoneEvent) {
      s.off(specificPhoneEvent, handleEvent);
    }
  };
}
